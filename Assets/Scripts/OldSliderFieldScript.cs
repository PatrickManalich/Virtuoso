...
public class OldSliderFieldScript : MonoBehaviour {
    ...
    //    startAid.transform.localScale = dummy.transform.localScale * 0.999f; // Prevents mesh clashing when overlaid
    //    endAid.transform.position = dummy.transform.position;
    //    endAid.transform.localScale = dummy.transform.localScale * 0.999f;   // Prevents mesh clashing when overlaid
    //}
    ...
    ///* Takes in a sample count and initializes the dummy sample count with this value. Necessary because cannot be intialized
    // * during Start() since DummyScript uses Start(). */
    //public void InitializeSampleCount(int sampleCount) { dummySampleCount = dummyScript.GetSampleCount(); }
